...
% Kalpurush (Windows system font), same family already used in this
% project's other two PDFs (features.tex, features-brief.tex) --
% consistent house style. Renderer=HarfBuzz + Script=Bengali get the
% correct OpenType shaping rules; Scale brings its x-height in line with
% the rest of the document. Only one weight ships for this family, so
% bold/italic silently fall back to the regular glyphs rather than a
% true bold or italic design -- the same tradeoff already accepted in
% this project's other Kalpurush-set documents.
