...
Two features requested by studying a reference app (`Bostro-Bangla`, a Next.js/MongoDB multi-vendor marketplace) for its floating AI chat widget and its seller-authored "posts" feature, then designing KenaBecha-specific versions rather than porting either as-is. Phase 51 is implemented (below); Phase 52 is still a plan, not yet built — following the project's established pattern, each ships as its own phase, built and verified before moving to the next.
