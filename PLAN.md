...
- **`TRUST_PROXY_HEADERS` unset in production.** Confirmed in the live `.env.prod`: it's absent, defaulting to `False`, while production sits behind Traefik. Every rate-limited endpoint is currently bucketing all visitors together by Traefik's container IP — one user's limit is everyone's limit. Set it to `true` in `.env.prod` and document it in `.env.prod.example` with the reason (Traefik, not optional). **Written down here but not actually applied** — re-discovered live during Phase 51 when a real user's assistant messages started 429ing after only two or three, caused by this session's own testing traffic sharing the same unscoped bucket. Actually fixed then: defaulted `true` directly in `docker-compose.prod.yml` (safe there specifically, since that file only ever runs behind Traefik, unlike the app's own conservative default), not just documented — closing the gap between "written in the plan" and "true in the running container" that let this sit unfixed since this phase was first drafted.
...
## Phases 51–52 — AI shopping assistant & shop posts

Two features requested by studying a reference app (`Bostro-Bangla`, a Next.js/MongoDB multi-vendor marketplace) for its floating AI chat widget and its seller-authored "posts" feature, then designing KenaBecha-specific versions rather than porting either as-is. Both are implemented.
...
### Phase 51 — Floating AI shopping assistant (implemented)
...
**What shipped, and the one design decision made while building rather than assumed above.** Provider is OpenAI directly (the user's own key, configured via `OPENAI_API_KEY`/`OPENAI_BASE_URL`/`OPENAI_MODEL` in `.env`, same `Settings` pattern as `GOOGLE_CLIENT_ID`) — kept configurable rather than hardcoded per the plan above, even though the provider choice itself is settled. The "never invent a listing" guarantee is enforced structurally, not just prompted: a third tool, `recommend_listings(listing_ids)`, is how the model declares its final picks separately from prose, and the backend intersects those ids against the union of everything a real `search_listings` call actually returned during that request (`assistant_service.run_assistant`) — anything else is silently dropped before it ever reaches the client, regardless of what the model claims. Admin settings (`enabled`, `system_prompt`) live under the `assistant` `site_settings` key exactly like `announcement`'s (`assistant_settings_service.py`, mirroring `announcement_service.py` field-for-field), edited from a new section on `/admin` (`AssistantEditor`, modeled on `AnnouncementEditor`). The widget hides on `/admin` and on the inbox thread page, the same route exclusion `MobileBottomNav` already uses and for the same reason (wrong audience / composer overlap).

Caught during live verification with a real key: the default system prompt initially let the model describe search results in prose and then ask "would you like me to see these?" instead of calling `recommend_listings` in the same turn — a reasonable conversational choice, and the structural guarantee correctly returned zero cards rather than showing anything unconfirmed, but a worse default experience than it should be. Fixed by making the system suffix explicit that recommending is not optional once specific listings are named ("they can't see anything until you do"), re-verified live afterward — a real search for "fish" now returns full `ListingOut`-shaped cards (images, tags, seller, shop) in the same turn the model describes them.

Backend: 11 new tests (`tests/test_assistant.py`) — the kill switch never constructs an OpenAI client while disabled, admin CRUD + audit-log-only-on-real-change, the tool-calling loop's mechanics against a scripted fake OpenAI stream (no real API calls in CI), the fabrication-rejection boundary specifically (a real but never-searched listing, a fabricated UUID, and a malformed id are all dropped while a properly-searched one comes through), a runaway tool-call loop terminating at the hard iteration cap, and rate-limit 429/Retry-After. Full suite: 277 passed. Frontend: `tsc --noEmit` clean. Verified against the real OpenAI API (not mocked) end-to-end: a plain greeting streams prose with an empty `listings` event; a real product query streams prose and a `listings` event carrying full, correctly-shaped `Listing` records ready for the unmodified `ListingCard`; disabling the toggle 503s without touching OpenAI at all.

**Two more bugs, both found live in production after the user actually clicked through it — not in local testing, which had neither.**

The first was self-inflicted and specific to this feature: with `OPENAI_API_KEY` still blank, `AsyncOpenAI(...)` raises at *construction* time in this SDK version, not just on first request — and that construction happened before `run_assistant`'s `try/except`, so the exception escaped the generator entirely after Starlette had already sent a 200 with `text/event-stream` headers. The client was left with a connection that just closed with zero bytes and no visible error, instead of the graceful `error` event every other failure path produces. Fixed by moving client construction inside the `try`, with a regression test scripting exactly that failure.

The second was older and unrelated to this phase's own code, just newly visible through it: `TRUST_PROXY_HEADERS` — flagged as a gap back in Phase 46, supposedly fixed there, but re-confirmed absent from the actual `.env.prod`/`docker-compose.prod.yml` when this session's own test traffic against the live assistant endpoint pushed a real user into 429s after only two or three messages, all sharing one rate-limit bucket keyed on Traefik's container IP rather than each visitor's real one. Actually closed this time — defaulted `true` directly in `docker-compose.prod.yml` rather than left as a `.env.prod` value someone has to remember to set, since that compose file only ever runs behind Traefik.

### Phase 52 — Shop posts: product-centric, not editorial (implemented)
...
**What shipped, and where it diverged from the plan above.** The actual follow-up request replaced "caption" with something more specific — a title plus a description the seller can select and re-style (bold/italic/underline, font, colour) — so `description_html` is sanitized rich text authored through **Tiptap** (this app's first WYSIWYG editor), not a plain-text caption. Storage is sanitized HTML at rest, not Markdown or Tiptap's JSON doc, to avoid inventing a formatting dialect or coupling storage to one editor library. This is the app's first user-generated-HTML-at-rest surface, so it got a real defense, not an afterthought: a new `nh3`-based sanitizer (`app/services/sanitize.py`) allow-lists a small tag set (`p br b strong i em u span`) and, since `nh3.clean()` doesn't parse `style="..."` values property-by-property, a hand-rolled CSS-property allow-list on top (`font-weight`, `font-style`, `text-decoration`, `font-family`, `color`, `background-color`, with `url()`/`expression()`/`javascript:` rejected in any value) — verified live against a payload containing a `<script>` tag, an `onclick` attribute, and a `background: url(javascript:...)` style, all three stripped while a sibling `color`/`font-family` declaration survived untouched. The Tiptap toolbar is deliberately built from exactly the tags that sanitizer allows, so the editor never lets a seller produce formatting that silently vanishes on save. `dompurify` runs a second pass client-side, immediately before every `dangerouslySetInnerHTML` render — defense in depth specifically because this is a novel surface and the server-side CSS filter is hand-written, not a battle-tested library, not a general policy applied elsewhere.

Two backend pieces the original plan didn't call out explicitly turned out to be required once the frontend was built against it: `GET /posts/shop/{shop_id}` (published-only for anyone but the owning shop's owner or staff) for the storefront's Posts tab, since the plan's `list_shop_posts` was owner-gated and had no public equivalent; and the public detail lookup moved from post id to **slug**, since the shareable `/posts/{slug}` URL the notification emails already point at can't resolve through an id-keyed endpoint. Rejection also grew a notification the plan's prose only implied — `NotificationType.post_rejected` tells the shop owner why, the same shape as `shop_new_post` tells followers a shop they follow posted something.

Moderation matches the plan's reuse of `DataTable`/`BulkBar` exactly, with one adjustment: bulk reject needs a single free-text reason applied to every selected post, which `BulkBar`'s static confirm-string can't collect, so the admin posts page renders its own reason dialog alongside `BulkBar` rather than stretching `BulkBar` itself to do something none of its other callers need.

Backend: 18 new tests (`tests/test_posts.py`) — sanitization end-to-end and as a standalone unit test, the resubmit-to-pending rule for both `published` and `rejected` posts (and that an already-`pending` post edited again just stays `pending`), cross-shop listing-ownership validation, the same listing linked from two separate posts (the explicit point of the feature — posting about a product again), follow-ranked feed order, notification fan-out to followers on approval and to the owner on rejection, the full moderation + audit trail, bulk moderation's per-item success/failure split, and visibility gating (pending/rejected posts 404 for a stranger, 200 for the owner or staff). Full suite: 296 passed. Frontend: `tsc --noEmit` clean. Verified against the real dev API end-to-end rather than just the test suite: created a post with the XSS payload above through the actual `/posts` endpoint, approved it as a moderator, and confirmed it appeared correctly in the feed, the storefront tab, and by-slug lookup, then cleaned the smoke-test rows out of the dev database.

**Caught after shipping, the same way Phase 51's two post-launch bugs were: by the user actually clicking through it.** The feed and moderation queue worked, but nothing pointed at them — the navbar's own seed data (Phase 47) was never touched, so `/posts` was reachable only by a direct URL, and the shop dashboard's per-shop "Posts" button was easy to miss with no standalone entry point. Fixed on all three real navigation surfaces at once: a new migration inserts a `nav.posts` link into the already-seeded `navbar` menu (right after "Browse Shops", ahead of the signed-in-only actions — targeted by `translation_key`, not `sort_order`, so it doesn't reshuffle anything an admin has since customized through the navigation editor), which surfaces on the desktop navbar and the mobile hamburger sheet for free since both already render whatever that menu returns; the persistent mobile bottom tab bar (a separate, hardcoded list, not admin-editable) gained its own `Posts` entry; and the `/dashboard` hub's sidebar gained a first-class "My Posts" row pointing at `/shops/dashboard/posts`, alongside the existing per-shop button rather than replacing it. `test_navigation.py`'s seed-reproduction test updated to match the new shipped shape; full suite still 296 passed, `tsc --noEmit` clean.
