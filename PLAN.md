...
- [ ] **Phase 9 — Design system foundation.** shadcn/ui + Tailwind CSS-variable theming, dark/light toggle (`next-themes`), color palette, typography/spacing/radius/z-index scales, motion presets. Infra + base component kit only, not a full page-by-page redesign yet.
- [ ] **Phase 10 — Tiered authentication: Google OAuth + full JU signup.** Supersedes the original placeholder "Phase 9 — Google OAuth" line below. Google one-click signup/login for buyers (no JU fields required), full JU-verified signup remains mandatory before opening a shop or creating a listing, "complete your profile" gate for Google-lite users who try to sell.
- [ ] **Phase 11 — Cart & orders, pickup/delivery fulfillment.** New cart/order data model, checkout flow (no payment integration — still a reserve-and-meet-up/arrange-delivery model), pickup vs. delivery choice on listings with conditional address collection.
- [ ] **Phase 12 — Public landing page.** Real homepage replacing the current placeholder, browsable without an account, built on the Phase 9 design system.
- [ ] **Phase 13 — Full application redesign pass.** Retrofit every pre-existing page (auth, listings, shops, inbox, profile, admin) onto the Phase 9 design system; mobile-first responsive audit and animation pass across the whole app.

See "Phase 9+ — UI/UX Redesign, Tiered Auth & Cart/Orders" below (before "## Context") for the detailed breakdown of these five phases, written 2026-08-01 per user request, planning-only — implementation proceeds phase by phase on confirmation.
